      ...
      <div class="page-tab-container">
        ...
        >
          <mt-tab-container-item id="offer">
            <mt-header fixed="fixed" title="待办事项">
              <mt-button icon="more" slot="right" @click="tools"></mt-button>
            </mt-header>
                  ...
                  >
                    <ul class="page-loadmore-list">
                      <mt-cell
                        ...
                        :value="item.insurant"
                        v-on:click.native="loadDetail(item)"
                      ></mt-cell>
...
export default {
  ...
  watch: {
    ...
    selected: function(val, oldVal) {
      ...
      if (val == "offer") {
        this.pageInfo.page = 1;
        this.getProjectInfo("newLoad", "selected");
      }
    }
  },
  methods: {
    ...
    getProjectInfo(type, path) {
      ...
      this.$axios.post(`api/vehicles/getVehicles`, options).then(res => {
        ...
        if (datas.code === 0) {
          ...
          console.log(
            `billist.length ${this.billlist.length} <----> total ${this.pageInfo.total}`
          );
